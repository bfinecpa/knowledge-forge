...
- RabbitMQ의 ack/nack과 prefetch(QoS)는 각각 무엇을 결정하나요? auto-ack의 위험은? (Kafka의 오프셋 커밋과 비교해서 설명해주세요)
...
- RabbitMQ에서 메시지 순서는 어디까지 보장되나요? 무엇이 순서를 깨뜨리나요? (Kafka의 파티션 단위 순서와 비교)
- Classic queue와 Quorum queue의 차이는? 미러링 큐가 왜 quorum queue로 대체되었나요? 네트워크 파티션(split-brain) 상황에서 각각 어떻게 동작하나요?

## RabbitMQ 심화 ⭐⭐⭐

- RabbitMQ 큐에 메시지가 계속 쌓일 때(적체) 원인 진단과 해소를 단계적으로 설명해주세요. (flow control, 메모리/디스크 alarm, lazy queue — Kafka 컨슈머 랙 대응과 비교)
- RabbitMQ 컨슈머의 처리량을 높이는 방법은? (prefetch 튜닝, 리스너 동시성, connection/channel 모델) 이때 순서 보장과 ack 처리는 어떻게 달라지나요?
- RabbitMQ에서 지연 메시지·스케줄링을 어떻게 구현하나요? (TTL + DLX vs delayed message exchange 플러그인) 같은 요구를 Kafka로 풀면 왜 어려운가요?
