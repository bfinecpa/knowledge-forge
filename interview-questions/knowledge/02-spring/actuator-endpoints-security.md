...
| 엔드포인트                                       | 운영 용도                                                                                                                                                                                                                                                                                                |
|---------------------------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `/actuator/health`                          | LB 헬스체크, K8s liveness/readiness probe의 대상. 무중단 배포에서 "트래픽 차단 신호"의 근거 ([graceful-shutdown-zero-downtime-deploy.md](graceful-shutdown-zero-downtime-deploy.md))                                                                                                                                         |
| `/actuator/metrics`, `/actuator/prometheus` | Prometheus가 주기 수집하는 지표의 원천 — `hikaricp.connections.active/pending`, `tomcat.threads.busy`, JVM 힙/GC. 풀 산정 검증과 고갈 진단의 데이터가 여기서 나온다 ([thread-pool-connection-pool-sizing.md](thread-pool-connection-pool-sizing.md), [hikaricp-connection-pool-exhaustion.md](hikaricp-connection-pool-exhaustion.md)) |
...
- **"SSRF와의 결합은?"** — 내부망 제한만 믿으면, 외부 노출 앱의 SSRF 취약점을 경유해 `http://localhost:9292/actuator/env`로 우회 접근 가능 — 인증 겹이 필요한 이유.
  상세: [ssrf-server-side-request-forgery.md](../16-security/ssrf-server-side-request-forgery.md)
