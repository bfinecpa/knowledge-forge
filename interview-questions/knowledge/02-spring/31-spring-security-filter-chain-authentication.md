...
- `SecurityContextHolder`는 **ThreadLocal** — 트랜잭션 커넥션 바인딩(`24-transaction-synchronization-connection-binding.md`)과 같은 메커니즘, 같은 함정: **@Async 새 스레드에는 전파되지 않는다**(대책: `DelegatingSecurityContextAsyncTaskExecutor` 또는 TaskDecorator 복사 — 읽기용 컨텍스트라 커넥션과 달리 복사가 안전).
