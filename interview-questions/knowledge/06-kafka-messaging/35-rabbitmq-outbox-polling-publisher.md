...
**CDC(Change Data Capture)**는 DB의 트랜잭션 로그를 구독해 변경을 감지하는 방식이다. Kafka 진영에는 Debezium + Kafka Connect라는 성숙한 경로가 있어 사실상 표준이다. 자세한 원리는 `12-transactional-outbox-cdc-kafka.md`에서 다룬다.
...
그리고 근본적으로, **컨슈머가 순서에 의존하지 않게 만드는 것**이 가장 견고하다. 이벤트에 버전을 넣고 오래된 것은 무시하는 방식이며, `30-rabbitmq-message-ordering.md`에서 다룬다.
