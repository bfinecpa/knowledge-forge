...
헷갈리기 쉬운 지점 하나. `mappedBy`가 붙은 쪽은 **FK 관리에 대해서만** 읽기 전용이다. `cascade`와 `orphanRemoval`은 **FK 관리가 아니라 생명주기 전파**라서, `mappedBy` 쪽(`Post.attachments`)에 붙이는 것이 정상이고 정상적으로 동작한다. "읽기 전용인데 왜 삭제가 되지?"라는 의문이 생긴다면, **두 개념이 다른 축**임을 다시 확인하면 된다. ([연관관계의 주인과 mappedBy](association-owner-and-mappedby.md))
...
이 불일치의 메커니즘과 `clearAutomatically`·`flushAutomatically`의 방향, 그리고 "**진짜 처방은 벌크 연산을 트랜잭션 끝이나 별도 트랜잭션으로 미는 것**"이라는 결론은 [벌크 연산과 영속성 컨텍스트](bulk-operation-persistence-context.md)에 정리되어 있다. 여기서 붙잡을 연결은 하나다.
