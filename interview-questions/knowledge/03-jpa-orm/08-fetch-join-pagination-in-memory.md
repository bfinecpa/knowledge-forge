...
- **Hibernate 7.4부터는 동작 자체가 바뀌었다** — limit/offset을 부모에 대한
  ...
  **현재 현업의 대다수 프로젝트는 여전히 메모리 페이징 구간**이다. 버전별 대응표는
  [N+1 탐지와 해결](04-n-plus-one-detection-and-fixes.md) §2-1에 정리돼 있다.
...
바인딩 등)은 [N+1 탐지와 해결](04-n-plus-one-detection-and-fixes.md) §2-3에 정리돼 있다.
...
[N+1 탐지와 해결](04-n-plus-one-detection-and-fixes.md) §3에 정리돼 있다 — **같은 장치가
이 문항의 회귀도 함께 막는다.**
...
끄는 행위에 가깝다. 자세한 근거는 [N+1 탐지와 해결](04-n-plus-one-detection-and-fixes.md)
§2-1에 있다. **(가산점 포인트)**
...
([LazyInitializationException](07-lazy-initialization-exception.md) §4 참고).
