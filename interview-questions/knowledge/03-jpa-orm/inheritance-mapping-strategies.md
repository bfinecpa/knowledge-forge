...
Hibernate에는 "초기화되지 않은 프록시를 로딩 없이 바로 삭제하는" 최적화 경로가 있는데, **엔티티에 상속 서브클래스가 있으면 이 최적화가 적용되지 않는다**(어느 테이블을 지워야 하는지 알려면 타입을 알아야 하므로). 즉 `delete(getReferenceById(id))`로 SELECT를 없애는 기법이 상속 계층에서는 기대대로 동작하지 않을 수 있다. **상속 매핑은 그 엔티티 하나의 문제로 끝나지 않는다** — 자세한 조건은 `find-by-id-vs-get-reference-by-id.md`에 있다.
