...
**함정 세 개**:
- **"영속성 컨텍스트가 알아서 걸러준다"는 전제.** ORM이 무언가를 대신 해준다는 인상 때문에 생기는 오해인데, 이 전제 위에서는 "그럼 JPA가 락을 잡겠네 → 커넥션이 고갈되겠네"까지 **틀린 결론이 자동으로 따라온다.** 뒤에서 보겠지만 커넥션 대기는 실제로 일어날 수 있으나 **이유가 완전히 다르다**(§1-6).
- **`try/catch`로 잡으면 끝났다고 믿는 것.** 예외를 잡는 것과 트랜잭션을 살리는 것은 별개다. 이 문항에서 실무 사고를 가르는 지점이 바로 여기다(§3).
- **"락을 걸었으니 제약은 없어도 된다"는 최적화.** 유니크 인덱스는 성능 부담이 아니라 **마지막 안전망**이고, 락이 덮지 못하는 경로가 반드시 존재한다(§4).

---

## 1. 첫 번째 못 — JPA는 유니크 제약을 검사하지 않는다
...
### 1-4. 검증은 flush 시점의 DB가 한다 — 예외의 3층 구조

flush가 일어나면 쌓아둔 INSERT가 DB로 나가고, 그때 **DB의 유니크 인덱스가** 중복을 발견한다. 이 실패가 우리 코드까지 올라오는 경로는 세 층이다.

```
[1층] DB (MySQL)
      ERROR 1062 (23000): Duplicate entry '7-3' for key 'uk_coupon_issue_user_coupon'
        ↓  JDBC 드라이버가 SQLException 으로 감싼다
[2층] 하이버네이트
      org.hibernate.exception.ConstraintViolationException
        ↓  Spring 의 예외 변환기(PersistenceExceptionTranslator)가 번역
[3층] Spring
      org.springframework.dao.DataIntegrityViolationException
        └ 그 하위 타입: DuplicateKeyException  ← 유니크/PK 중복일 때 이걸로 좁혀서 온다
```
...
`DuplicateKeyException`까지 좁혀 잡으면 "중복"만 정확히 골라낼 수 있다. `DataIntegrityViolationException`은 NOT NULL 위반, 외래 키 위반, 길이 초과까지 포괄하므로 **너무 넓게 잡으면 전혀 다른 버그를 "이미 발급됨"으로 오인해 삼켜버린다.**
...
// ✅ AFTER — 중복만 정확히 골라내고 나머지는 그대로 올려보낸다
catch (DuplicateKeyException e) {
    return IssueResult.ALREADY_ISSUED;
}
// (더 엄밀히 하려면 제약 이름까지 확인한다 — §5장 꼬리질문 참고)
```

### 1-5. 그래서 실제 시간 순서는 이렇게 흐른다

같은 사용자의 요청 두 개가 동시에 들어온 상황을 시간축에 그리면 이렇다.

```
시각   스레드 A                              스레드 B
────────────────────────────────────────────────────────────────────
 t1   exists(7,3) → SELECT → false
 t2                                        exists(7,3) → SELECT → false   ← 둘 다 통과
 t3   save() → 쓰기 지연 저장소에 적재
 t4                                        save() → 쓰기 지연 저장소에 적재
 t5   커밋 → flush → INSERT (7,3)  성공
 t6                                        커밋 → flush → INSERT (7,3)
 t7                                        ✖ Duplicate entry → 예외
```

**t1~t4 구간에서 JPA는 아무것도 막지 않았다.** 막은 것은 t7의 DB 유니크 인덱스 하나뿐이다. 이 그림이 이 문항의 전부라고 해도 과언이 아니다.

### 1-6. "커넥션이 고갈된다"는 직관 — 결론은 스칠 수 있으나 이유가 다르다

"동시 INSERT가 몰리면 커넥션이 부족해진다"는 답이 흔히 나온다. **JPA가 락을 잡아서가 아니다.** 실제로 대기가 생긴다면 원인은 **DB 쪽(InnoDB)에 있다.**

InnoDB는 유니크 인덱스에 값을 넣기 전에 **같은 값이 이미 있는지 인덱스 레코드를 확인**하는데, 그 값을 만든 트랜잭션이 아직 커밋도 롤백도 안 한 상태라면 **결과를 확정할 수 없다.** 커밋되면 중복이고 롤백되면 중복이 아니기 때문이다. 그래서 **뒤에 온 INSERT는 앞 트랜잭션이 끝날 때까지 대기**한다. 이 대기 동안 그 요청은 **DB 커넥션을 붙잡고 있으므로**, 앞 트랜잭션이 길면(외부 API 호출을 끼워 넣었다든가) 대기 줄이 길어지고 커넥션 풀이 마를 수 있다.

즉 **"커넥션 고갈"이라는 현상은 가능하지만, 그 원인은 JPA의 락이 아니라 DB의 유니크 인덱스 검사 대기**다. 처방도 그래서 달라진다 — "JPA 락을 없애자"가 아니라 **"INSERT를 포함한 트랜잭션을 짧게 유지하고, 외부 호출을 트랜잭션 밖으로 빼자"**가 된다. (같은 이야기가 [낙관적 락 vs 비관적 락](optimistic-vs-pessimistic-lock.md)의 "비관적 락의 진짜 대가는 커넥션 점유"와 정확히 겹친다.)

---

## 2. check-then-act는 원자적이지 않다 — 사전 검사는 UX용이다

### 2-1. 왜 반드시 깨지는가

`exists()`와 `save()`는 **두 개의 문장**이고, 두 문장 사이에는 언제나 다른 스레드가 끼어들 틈이 있다. 이것을 **check-then-act**(확인하고 나서 행동하기) 패턴이라 부르고, 동시성 환경에서 안전하지 않은 대표적 형태다. §1-5의 t1~t4가 정확히 그 틈이다.

여기서 사람들이 마지막으로 붙잡는 희망이 **"트랜잭션 격리 수준을 올리면 되지 않나"**인데, **막을 수 없다.** 격리 수준이 다루는 것은 **"이미 존재하는 행을 어떻게 보느냐"**이지, **"아직 존재하지 않는 행을 남이 만들려 하는 것"**이 아니다. REPEATABLE READ에서 두 트랜잭션은 각자의 스냅샷을 보고 둘 다 "없다"고 판단한다 — 스냅샷은 **상대의 미커밋 INSERT를 정의상 보여주지 않기 때문**이다. (SERIALIZABLE은 §5 꼬리질문에서 따로 다룬다.)

### 2-2. 그럼 사전 검사를 지워야 하나 — 지우지 말고 **역할을 바꿔라**
...
그런데 **AFTER 코드에도 아직 큰 구멍이 남아 있다.** 2층에서 예외가 터졌을 때 그걸 어떻게 처리할 것인가 — 여기서 대부분이 다음 함정에 빠진다.

---

## 3. 예외를 잡아도 트랜잭션은 살아나지 않는다 — rollback-only 마킹

### 3-1. 증상: "분명히 catch 했는데 메서드 끝에서 예외가 터진다"
...
```
1. save() → flush → Duplicate entry → DataIntegrityViolationException
...
### 3-2. 왜 그런가 — 두 층에서 동시에 일어나는 일

**첫째, JPA 명세 차원의 규칙이다.** `EntityManager` 연산이 예외를 던지면 **영속성 제공자(하이버네이트)는 그 트랜잭션을 롤백 대상으로 표시해야 한다**고 명세가 규정한다(조회 결과 없음이나 락 타임아웃 같은 소수 예외를 제외하고). 이유는 단순하다. 제약 위반이 난 시점에 **하이버네이트 세션 안의 상태와 DB의 상태가 어긋났고**, 하이버네이트는 어긋난 정도를 정확히 알 수 없다. 쓰기 지연 저장소에 남은 SQL 중 무엇이 나갔고 무엇이 안 나갔는지, 1차 캐시의 어떤 엔티티가 유효한지 보장할 수 없으므로 **"이 세션은 오염됐다. 더 쓰지 말고 버려라"**가 유일하게 안전한 선택이다.

**둘째, Spring 트랜잭션의 rollback-only 플래그다.** 위 규칙의 결과로 현재 트랜잭션에 **"커밋 금지"라는 표식**이 찍힌다. 이 표식은 **`try/catch`로 지울 수 없다.** 예외 객체를 잡는 것과 트랜잭션의 상태 플래그는 완전히 다른 층위이기 때문이다.

> 비유: **경보기가 울린 것을 손으로 막았다고 해서, 이미 잠긴 방화 셔터가 열리지는 않는다.** `catch`는 소리를 막았을 뿐이고, 셔터(rollback-only)는 별도 장치다.

그리고 이것이 **커밋 시점까지 조용하다**는 점이 이 버그를 특히 고약하게 만든다. 예외 메시지의 "silently rolled back"이 정확히 그 뜻이다 — **catch 블록 안에서 한 모든 DB 작업이 성공한 것처럼 보이다가 마지막에 통째로 사라진다.**

### 3-3. 처방 ① — 후속 작업을 `REQUIRES_NEW`로 분리한다
...
public class CouponIssueFacade {
    ...
    public IssueResult issue(Long userId, Long couponId) {
        ...
        } catch (DuplicateKeyException e) {           // 동시성으로 뚫린 경우
...
### 4-2. 그리고 더 근본적인 문제 — 락을 아예 거치지 않는 진입 경로
...
[App]       분산 락 / 비관적 락                 ← 강력하지만 §4-1·4-2 경로에서 뚫림
...
## 5. 제3의 선택지 — 원자적 upsert로 예외 자체를 없앤다

"락으로 막느냐, 예외를 잡느냐"는 이분법에는 세 번째 답이 있다. **DB에게 "있으면 무시하고 없으면 넣어라"를 한 문장으로 시키는 것**이다. 확인과 삽입이 한 문장 안에서 일어나므로 **틈이 원천적으로 존재하지 않는다.**
...
### 5-2. Spring Data JPA에서 쓰는 법

JPQL에는 이런 구문이 없으므로 **네이티브 쿼리**로 쓴다.
...
public IssueResult issue(Long userId, Long couponId) {
    ...
    // MySQL 의 ON DUPLICATE KEY UPDATE 는 영향 행 수가 이렇게 나온다:
    //   1 = 새로 INSERT 됨,  2 = 실제로 UPDATE 됨,  0 = 값이 그대로라 변경 없음
    // 위처럼 "아무것도 바꾸지 않는" 관용구를 쓰면 중복일 때 0 이 돌아온다.
    return affected == 1 ? IssueResult.SUCCESS : IssueResult.ALREADY_ISSUED;
}
```

**얻는 것이 두 가지다.** ① 왕복이 한 번이라 가장 빠르고 락 비용이 0이다. ② **예외가 아예 발생하지 않으므로 §3의 rollback-only 문제까지 통째로 우회된다.** 이 두 번째 효과가 특히 크다 — 예외 처리 설계 자체가 사라진다.

### 5-3. 대가 — 영속성 컨텍스트를 우회한다

네이티브 쿼리는 **1차 캐시를 지나치지 않고 곧장 DB로 간다.** 이것은 벌크 연산(`@Modifying` UPDATE/DELETE)과 정확히 같은 성질이며, 같은 함정을 가진다([벌크 연산과 영속성 컨텍스트](bulk-operation-persistence-context.md)).
...
**추가로 잃는 것들**: 하이버네이트의 엔티티 리스너·감사(`@CreatedDate` 등)·`@Version` 증가가 동작하지 않고, SQL이 DB 벤더에 묶인다(MySQL과 PostgreSQL 구문이 다르다). 그래서 upsert는 **"엔티티 생명주기가 단순한 관계 테이블"**에 가장 잘 맞는다 — 쿠폰 발급 이력, 좋아요, 팔로우, 조회 기록처럼 **"있으면 그만, 없으면 만든다"가 전부인 행**이 그렇다.

### 5-4. `INSERT IGNORE`의 함정 (가산점 포인트)
...
또 하나: **한 테이블에 유니크 인덱스가 여러 개일 때** upsert 구문은 "어느 제약에서 충돌했는지"를 구분해 주지 않는다(PostgreSQL은 `ON CONFLICT (컬럼)`으로 지정 가능, MySQL은 불가). 의도한 제약이 아닌 다른 제약에서 충돌한 것을 **성공으로 오인**할 수 있으므로, 유니크 인덱스가 여럿인 테이블에서는 신중해야 한다.
...
**upsert는 이 축의 바깥에 있다.** 충돌 여부와 무관하게 왕복 한 번이고 예외도 없으므로, **요구사항이 "있으면 무시"로 정확히 표현되는 경우엔 빈도를 따질 필요조차 없다.** 다만 §5-3의 대가(영속성 컨텍스트 우회, 벤더 종속)를 받아들일 수 있어야 한다.
...
- **IDENTITY(MySQL AUTO_INCREMENT)**: id를 DB가 만들어주므로 쓰기 지연이 불가능하다. `save()` 호출 즉시 INSERT가 나가고, **그 줄에서 바로 예외가 터진다.**
- **SEQUENCE/TABLE**: id를 미리 확보할 수 있어 INSERT를 쌓아둔다. 예외는 **커밋 직전 flush 시점**에 터지고, 스택 트레이스가 `save()` 호출부를 가리키지 않아 원인 파악이 어렵다.

**진단하려다 헷갈리기 쉬운 점**: `saveAndFlush()`나 JPQL 실행은 강제로 flush를 유발하므로 **엉뚱한 줄에서 예외가 터진 것처럼 보인다.** "코드상 순서와 실제 SQL 실행 순서가 다르다"는 것이 영속성 컨텍스트의 기본 성질이다([영속성 컨텍스트와 변경 감지](persistence-context-dirty-checking.md)).
...
- **데드락, 락 타임아웃, 낙관적 락 충돌**은 일시적이다. 같은 작업을 다시 하면 이번엔 성공할 수 있다 — 그래서 재시도가 정답이다([낙관적 락 vs 비관적 락](optimistic-vs-pessimistic-lock.md)).
...
**(주의)** 다만 `@Retryable`을 `DataIntegrityViolationException`에 걸어두는 코드가 실제로 존재하는데, §3의 rollback-only 문제와 겹치면 **"영원히 실패하며 커넥션만 소모하는 루프"**가 된다. 재시도 대상 예외는 **일시적 실패로 한정**해야 한다.

### "트랜잭션 격리 수준을 SERIALIZABLE로 올리면 유니크 제약 없이 막을 수 있나요?" (시니어 변별 포인트)

**이론적으로는 막히지만, 실무적으로는 답이 아니다.**

먼저 **REPEATABLE READ로는 못 막는다.** 격리 수준은 "이미 존재하는 행을 어떻게 볼 것인가"를 규정할 뿐, "아직 없는 행을 남이 만드는 것"을 다루지 않는다. 두 트랜잭션은 각자의 스냅샷에서 "없다"를 읽고 둘 다 통과한다.

**SERIALIZABLE로 올리면 동작 방식이 달라진다.** MySQL(InnoDB)은 일반 SELECT를 잠금 읽기로 승격시키는데, 그러면 두 트랜잭션이 서로의 공유 락 때문에 진행하지 못해 **데드락으로 한쪽이 죽는다** — "막았다"기보다 **"데드락으로 실패시켰다"**에 가깝다. PostgreSQL은 직렬화 이상을 탐지해 한쪽에 직렬화 실패를 던진다.

**어느 쪽이든 실무 답이 아닌 이유가 셋이다.** ① 격리 수준은 **트랜잭션 전체에 걸리므로**, 이 한 줄을 지키려고 무관한 모든 쿼리의 동시성까지 희생한다. ② 데드락·직렬화 실패는 **재시도 로직을 강제**하므로 유니크 제약보다 처리가 복잡해진다. ③ 무엇보다 **§4-2의 "락을 안 거치는 경로"는 여전히 안 막힌다** — 격리 수준은 트랜잭션을 쓰는 코드에만 적용되기 때문이다.
