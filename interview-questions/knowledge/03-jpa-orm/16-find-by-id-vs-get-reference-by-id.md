...
**이 문서의 지도**: 본문은 축 세 개로 되어 있다. **§1은 "두 메서드가 무엇이 다른가"** — 프록시가 무엇이고, SQL이 어떻게 줄고, 같은 절약이 숨어 있는 두 번째 자리인 `deleteById`는 어떤 조건에서 통하는가. **§2는 "프록시가 만드는 함정"** — 같은 실수인데 예외 이름이 둘로 갈리는 이유, 실패 지점이 얼마나 멀리 밀려나는지, 그리고 프록시가 `getClass()`와 `==`를 배신하는 문제. **§3은 "그래서 어떻게 판단하는가"** — 선택 기준 한 줄, 배치용 완성형 패턴, 그리고 그다음 병목까지.

버전에 의존하는 서술은 **Hibernate ORM 6.6 / Spring Data JPA 3.5 / Spring Boot 3.x 기준**이다. 내부 동작은 실제 클래스 이름과 함께 적었으니 의심스러우면 소스에서 직접 확인하면 된다.

---

## 1. 두 메서드의 차이 — SELECT를 지금 날리느냐, 미루느냐

### 1-1. 프록시란 무엇인가 — 그리고 두 메서드가 갈리는 지점

먼저 용어부터 세운다. **프록시(proxy)**는 "대리인"이라는 뜻으로, 여기서는 **Hibernate가 런타임에 만들어 낸 `Member`의 가짜 자식 클래스 인스턴스**다. 겉모습은 `Member`지만 속은 비어 있고 `id`와 "나는 아직 안 채워졌다"는 표시만 들고 있다. 그러다 **`getName()` 같은 실제 필드 접근이 처음 일어나는 순간** DB로 SELECT를 날려 자기 속을 채운다. 이것을 **프록시 초기화**라고 부른다.

비유하자면 프록시는 **주소만 적힌 택배 송장**이다. 송장만 있어도 "이 물건이 어디로 갈 것인지"는 알 수 있고, 그 정보만 필요한 일(연관관계 FK 채우기, 삭제 대상 지정)은 상자를 뜯지 않고도 처리된다. 상자 안의 내용물이 필요해지는 순간에야 비로소 배송을 요청한다.
...
Spring Data의 구현도 딱 한 줄씩이다. `SimpleJpaRepository.findById`는 `entityManager.find(...)`를, `getReferenceById`는 `entityManager.getReference(getDomainClass(), id)`를 그대로 부른다. **두 메서드의 모든 차이는 JPA 표준 API 두 개의 차이에서 나온다** — Spring Data가 따로 하는 일은 없다.
...
**"원인 지점과 발현 지점이 멀어지면 장애 분석이 어려워진다"** — 이 한 문장이 프록시의 대가 전부다. 이 "멀어진다"가 실제로 얼마나 멀어지는지는 §2-2에서 스택 트레이스로 확인하고, 그 대가를 관리하는 방법은 §3 전체의 주제다.

### 1-4. 같은 낭비가 숨어 있는 두 번째 자리 — `deleteById`
...
**왜 SELECT가 사라지는가**는 두 단계로 갈린다. 두 층이 모두 통과해야 SELECT가 실제로 없어진다.

**① Spring Data 쪽** — `delete(T entity)`의 내부는 이렇다.

```java
// Spring Data JPA — SimpleJpaRepository.delete (요지)
if (entityManager.contains(entity)) {
    entityManager.remove(entity);       // ← 프록시는 이미 영속 상태라 여기로 빠진다
    return;
}
Class<?> type = ProxyUtils.getUserClass(entity);
// 준영속 엔티티였다면 아래에서 find 로 존재를 확인한 뒤 merge → remove
...
**② Hibernate 쪽** — Hibernate에는 **"초기화되지 않은 프록시를 로딩 없이 바로 지우는" 최적화 경로**가 실제로 구현되어 있다(`DefaultDeleteEventListener.optimizeUnloadedDelete`). 이 경로를 타면 프록시를 초기화하지 않고 `EntityDeleteAction`만 큐에 등록한다.

### 1-5. 그 최적화가 조용히 꺼지는 조건 (여기가 진짜 포인트)

**항상 되는 게 아니다.** Hibernate는 `canBeDeletedWithoutLoading()`이 참일 때만 로딩 없이 지운다. 이 메서드는 아래 조건을 **전부** 확인하고, 하나라도 걸리면 프록시를 초기화하고(= SELECT를 날리고) 평소 경로로 지운다.

| 조건 | 실무에서 언제 걸리나 |
|---|---|
| 세션에 커스텀 `Interceptor`가 없을 것 | 직접 등록한 `Interceptor`가 있을 때 |
| 엔티티가 Hibernate `Lifecycle` 인터페이스를 구현하지 않을 것 | 레거시 코드에서 드물게 |
| 엔티티에 **상속 서브클래스가 없을 것** | `@Inheritance` 매핑이 걸려 있으면 탈락 |
| **cascade delete 설정이 없을 것** | 자식까지 지워야 하면 부모 상태를 알아야 하므로 |
| **`@NaturalId`가 없을 것** | 자연 키를 선언한 엔티티 |
| PK를 참조하지 않는 컬렉션이 없을 것 | `@JoinColumn`이 PK가 아닌 컬럼을 가리킬 때 |
| **`@PreRemove` / `@PostRemove` 콜백이 없을 것** | 콜백에 엔티티를 넘겨야 하니 로딩이 필요하다 |
| **PRE_DELETE / POST_DELETE 이벤트 리스너가 없을 것** | 아래 참고 |

마지막 항목이 실무에서 가장 자주 걸린다. Hibernate 소스의 `hasCustomEventListeners()` 위에 달린 주석이 두 가지를 콕 집어 언급한다 — **"Bean Validation adds a PRE_DELETE listener and Envers adds a POST_DELETE listener."** 즉 `spring-boot-starter-validation`이 클래스패스에 있는 흔한 프로젝트에서는 이 최적화가 **꺼져 있을 수 있다.**
...
### 1-6. `deleteById`에는 "존재하지 않으면 조용히 넘어간다"는 성질이 있다

바꾸기 전에 알아야 할 **의미 차이**가 하나 있다. 현재 Spring Data의 `CrudRepository.deleteById` javadoc은 이렇게 적고 있다 — *"If the entity is not found in the persistence store it is silently ignored."* 코드에서도 `findById(id).ifPresent(...)`이므로 **없는 `id`를 지우려 하면 아무 일도 없이 끝난다.**
...
### 2-1. 같은 실수인데 예외 이름이 갈린다 — `EntityNotFoundException` vs `LazyInitializationException`

프록시를 쓰다 사고가 나면 **원인은 하나인데 이름이 두 개**로 나타난다. 이걸 모르면 검색 키워드를 잘못 잡아 엉뚱한 곳을 파게 된다.

**두 예외가 갈리는 이유를 한 줄로 요약하면 이렇다.** `EntityNotFoundException`은 **DB에 물어볼 수는 있었는데 답이 빈** 경우고, `LazyInitializationException`은 **물어볼 창구(세션)가 이미 닫혀서 물어보지도 못한** 경우다. 그러니까 갈림길은 "행이 있느냐 없느냐"가 아니라 **"세션이 살아 있느냐"가 먼저**다.

같은 행동 — 프록시의 필드를 건드리는 것 — 에서 시작해 두 갈래로 나뉘는 판정 흐름을 그리면 이렇다.

```text
             프록시의 필드에 접근한다   (예: member.getName())
                            │
                            ▼
                초기화(initialize)를 시도한다
                            │
              세션(EntityManager)이 살아 있는가?
                            │
        ┌───────────────────┴───────────────────┐
        │ 아니오 — 이미 닫혔다                  │ 예 — 열려 있다
        ▼                                       ▼
  DB에 물어볼 창구가 없다              select ... from member where id = 999
        │                                       │
        ▼                          ┌────────────┴────────────┐
  LazyInitializationException      │ 행이 있다               │ 행이 없다
  "Could not initialize proxy      ▼                         ▼
   [...Member#999] - no session"   프록시에 값이 채워진다    EntityNotFoundException
                                   (SELECT 1회, 정상 동작)   "Unable to find
        ↑ 행이 있어도 난다.                                    ...Member with id 999"
          원인은 "행"이 아니라 "세션"이다.
```

Hibernate 6.6 소스에서 두 메시지가 나오는 자리는 각각 이렇다. `LazyInitializationException`은 `AbstractLazyInitializer.initialize()`가 세션 상태를 검사하다 던지고(세션이 `null`이면 `- no session`, 닫혀 있으면 `- the owning session was closed`), `EntityNotFoundException`은 로딩을 마친 뒤 `checkTargetState()`에서 대상이 `null`인 것을 보고 `EntityNotFoundDelegate`가 던진다. Spring Boot는 JPA로 부트스트랩하므로 이 델리게이트가 `JpaEntityNotFoundDelegate`이고, 메시지는 `"Unable to find " + entityName + " with id " + id`가 된다.

경우를 전부 늘어놓으면 다음 표가 된다.
...
### 2-2. "실패 지점이 멀어진다"는 말을 스택으로 확인한다

§1-3의 "대가"는 추상적으로 들린다. 실제 스택 트레이스를 보면 무엇이 문제인지 분명해진다. 아래 코드에서 잘못된 `id`가 들어오는 자리와 예외가 터지는 자리를 표시해 두었다.
...
public class OrderService {
    ...
    @Transactional
    public ReceiptResponse createOrder(Long memberId, int amount) {          // 30행
        Member member = memberRepository.getReferenceById(memberId);         // 34행 ← 진짜 원인
        Order  order  = orderRepository.save(new Order(member, amount));     // 36행
        return buildReceipt(order);                                          // 41행
    }

    private ReceiptResponse buildReceipt(Order order) {                      // 85행
        return new ReceiptResponse(order.getId(),
                                   order.getMember().getName());             // 88행 ← 여기서 터진다
    }
}
```

**경우 A — 트랜잭션 안에서 프록시를 초기화한 경우.**

```text
jakarta.persistence.EntityNotFoundException: Unable to find com.example.member.Member with id 999
    at org.hibernate.proxy.AbstractLazyInitializer.checkTargetState(AbstractLazyInitializer.java:304)
    at org.hibernate.proxy.AbstractLazyInitializer.initialize(AbstractLazyInitializer.java:187)
    at org.hibernate.proxy.AbstractLazyInitializer.getImplementation(AbstractLazyInitializer.java:...)
    at org.hibernate.proxy.pojo.bytebuddy.ByteBuddyInterceptor.invoke(ByteBuddyInterceptor.java:...)
    at com.example.member.Member$HibernateProxy.getName(Unknown Source)
    at com.example.order.OrderService.buildReceipt(OrderService.java:88)     ◀── 스택이 가리키는 곳
    at com.example.order.OrderService.createOrder(OrderService.java:41)
    ...
```

**스택은 88행을 가리킨다. 잘못된 `id`가 들어온 34행은 스택 어디에도 없다.** `createOrder` 프레임은 있지만 그 프레임의 줄 번호는 41행(호출 지점)이지 34행이 아니다. 다행히 메시지에 `id 999`가 찍히므로 "999번 회원이 없었다"까지는 알 수 있지만, **그 999가 어디서 왔는지**는 스택이 알려주지 않는다. 값이 요청 본문·CSV 한 줄·다른 서비스의 응답에서 왔다면 추적은 거기서부터 시작이다.

`findById`를 썼다면 34행에서 `Optional.empty()`가 나오고 그 줄의 `orElseThrow`가 도메인 예외를 던진다. **원인과 발현이 같은 줄에 있다.** 이것이 "존재 검증을 호출 지점에서 한다"의 실질적 의미다.

**경우 B — 프록시가 트랜잭션 밖으로 나가 직렬화 단계에서 터진 경우.** OSIV를 끈 환경에서 서비스가 엔티티(또는 프록시를 품은 엔티티)를 그대로 반환하면 여기까지 밀린다.

```text
org.hibernate.LazyInitializationException:
        Could not initialize proxy [com.example.member.Member#999] - no session
...
**여기서는 애플리케이션 코드가 스택에 한 줄도 없다.** 컨트롤러 핸들러 메서드는 이미 정상 리턴했고, 예외는 그 뒤 **응답 본문을 JSON으로 쓰는 도중** 터졌다. `@ExceptionHandler`가 호출될 수는 있지만, Jackson이 이미 출력 버퍼에 일부를 흘려보내 응답이 커밋된 뒤라면 **클라이언트는 200 상태에 잘린 JSON을 받는다.** 로그에는 예외가 남는데 API 응답은 "성공"으로 보이는, 가장 진단하기 나쁜 형태다.

**두 경우를 관통하는 교훈**은 하나다 — **프록시는 "값이 필요해지는 지점"까지 실패를 미룬다. 그 지점이 내 코드에서 멀수록 진단 비용이 커진다.** 그래서 처방은 예외를 잡는 것이 아니라 **경계를 세우는 것**이고(프록시는 트랜잭션 안에서 소비한다), 배치처럼 프록시를 꼭 써야 하는 자리에서는 **실패를 미루는 대신 미리 목록으로 뽑아 두는 것**이다(§3-2).

### 2-3. 프록시는 `getClass()`와 `==`를 배신한다

`getReferenceById`가 돌려준 것은 `Member`가 아니라 **`Member`를 상속한 런타임 생성 클래스**다. Hibernate 6은 이 클래스의 이름을 `원본_클래스_이름 + "$HibernateProxy"`로 **고정해서** 만든다(`ByteBuddyProxyHelper.buildProxy`). 그래서 실제로 찍어 보면 이렇게 나온다.
...
proxy.getClass().getName()           // "com.example.member.Member$HibernateProxy"
proxy.getClass().getSimpleName()     // "Member$HibernateProxy"
proxy.getClass().getSuperclass()     // class com.example.member.Member
```

> **버전 주의**: Hibernate 5.x는 프록시 클래스 이름 뒤에 **무작위 접미사**를 붙였다(`Member$HibernateProxy$aB3xK1`). 6.x에서 고정 이름으로 바뀌었으므로, 옛 블로그 글의 출력값과 지금 로그가 달라 보인다고 놀랄 필요는 없다. **어느 쪽이든 `Member.class`와 같지 않다는 사실은 동일하다.**
...
real.getClass();                 // class com.example.member.Member
proxy.getClass();                // class com.example.member.Member$HibernateProxy
                                 //   → getClass() 비교가 false 를 만든다
...
### 2-4. 처방 — `instanceof` 또는 실질 클래스 비교

처방은 두 갈래다. 둘 다 **`getClass()` 직접 비교를 걷어내는 것**이 요지다.
...
> 이 주제는 [엔티티의 equals/hashCode](25-entity-equals-and-hashcode.md) 문항과 **뿌리가 같다.** "JPA 엔티티의 `equals`/`hashCode`를 어떻게 구현하나요?"라는 질문이 나오면 답의 뼈대는 셋이다 — **① 비즈니스 키가 있으면 그것으로, 없으면 `id`로 ② `getClass()` 대신 `instanceof` 또는 실질 클래스 비교(프록시 때문에) ③ `hashCode`는 `id`에 의존하지 않는 고정값(저장 전후로 해시가 바뀌면 안 되므로)**. 이 문서에서 본 프록시의 성질이 그 문항의 ②를 그대로 설명한다.

---

## 3. 판단과 확장 — 어디서 검증하고, 어디까지 내려갈 것인가

### 3-1. 선택 기준을 한 문장으로 고정한다
...
**① 왜 `findAllById`가 아니라 id 프로젝션인가.** `findAllById(ids)`도 쿼리 한 번으로 존재를 확인할 수 있어 훨씬 간단하다. 다만 **엔티티를 통째로 로딩해 1차 캐시에 올린다.** 회원 수가 수천 명 수준이면 `findAllById`가 실용적이고(그리고 이미 로딩되었으므로 §3-7에 의해 `getReferenceById`가 실제 엔티티를 그대로 돌려준다 — 결과적으로 추가 SELECT도 없다), 수십만이면 id만 뽑는 편이 안전하다. **판단 기준은 "검증 대상 집합의 크기"**다.

**② 누락분을 예외로 끝내지 않는 이유.** 배치에서 10만 건 중 3건이 잘못됐다고 전체를 실패시키면 재실행 비용이 크다. **어떤 `id`가 왜 빠졌는지 리포트로 남기고 나머지를 처리**하는 편이 대개 낫다. 이것이 `getReferenceById`의 대가인 "발현 지점이 멀어진다"(§2-2)를 실질적으로 상쇄하는 장치다 — **실패를 나중에 발견하는 대신, 아예 미리 목록으로 뽑아 둔다.**
...
**④ `em.clear()` 이후에도 `getReferenceById`는 안전하다.** 컨텍스트를 비우면 기존 프록시는 준영속이 되지만, 다음 루프에서 `getReferenceById`를 다시 부르면 새 프록시가 만들어진다. 다만 **`clear()` 이전에 만든 프록시를 `clear()` 이후에 초기화하려 하면 `LazyInitializationException`**이 난다(§2-1 표의 세 번째 행과 같은 상황이다).

### 3-3. 배치의 진짜 병목은 그다음이다 — `IDENTITY`가 JDBC 배치를 무력화한다
...
이유는 간단하다. `IDENTITY`는 **DB가 INSERT를 실행해야 비로소 `id`가 정해진다.** 그런데 JPA는 `persist()` 시점에 엔티티를 영속성 컨텍스트의 1차 캐시에 `id`를 키로 넣어야 한다. **`id`를 얻으려면 INSERT를 지금 당장 실행할 수밖에 없고**, 그래서 Hibernate는 `IDENTITY` 엔티티에 대해 **쓰기 지연을 포기하고 `persist()`마다 즉시 INSERT를 날린다.** 모아 둘 문장이 없으니 배치도 없다. Hibernate 소스에서는 이것을 "early insert"라 부르며, `ActionQueue.addInsertAction()`이 `insert.isEarlyInsert()`인 경우 큐에 쌓지 않고 그 자리에서 실행한다.
...
JPA를 유지한다면 최소한 **청크마다 `flush()` + `clear()`**로 영속성 컨텍스트가 무한히 커지는 것을 막아야 한다(§3-2 코드 참고). 이걸 안 하면 SELECT를 없앤 보람도 없이 OOM으로 끝난다.

> 이 절은 고난이도의 **"대량 데이터 INSERT 시 JPA의 한계와 JDBC batch / `IDENTITY` 문제"** 문항의 예고편이다. 이번 문항에서 "`getReferenceById`로 SELECT를 없앤다"까지 말한 뒤 **"다만 그건 절반이고, 남은 INSERT는 `IDENTITY`면 배치가 안 걸리므로 시퀀스나 JdbcTemplate으로 내려가야 합니다"**를 덧붙이면, 두 문항을 하나로 꿴 사람이라는 신호가 된다.
...
### 3-6. `getOne` → `getById` → `getReferenceById`로 이름이 두 번 바뀐 이유 (가산점 포인트)

같은 기능이 세 번 이름을 갈아입었다. 처음엔 `getOne`이었고, 다음이 `getById`, 지금이 `getReferenceById`다. **앞의 둘은 deprecated 되었고 최종 정착이 `getReferenceById`다.** 실제로 `SimpleJpaRepository`에서 `getOne`과 `getById`는 둘 다 `getReferenceById(id)`를 그대로 호출하는 한 줄짜리 메서드다.
...
**면접에서의 의미**: 오래된 코드에서 `getOne`을 보면 그건 "옛날 `findById`"가 아니라 **지금의 `getReferenceById`와 정확히 같은 것**이고, 따라서 §2의 지뢰를 그대로 안고 있다는 뜻이다.

### 3-7. 이미 1차 캐시에 있으면 `getReferenceById`는 프록시를 주지 않는다 (가산점 포인트)
...
**실무적 함의 두 개.** ① `getReferenceById`를 썼다고 해서 반드시 프록시가 나온다고 가정하고 코드를 짜면 안 된다(§2-3의 `equals` 문제가 **어떤 실행 경로에서는 재현되고 어떤 경로에서는 안 되는** 이유가 이것이다). ② 반대로 **검증 단계에서 `findAllById`로 미리 로딩해 두면 이후 `getReferenceById`는 자동으로 실제 엔티티를 준다** — §3-2의 ①에서 말한 그 얘기다.

### 3-8. `@ManyToOne(fetch = LAZY)` 필드가 사실 같은 물건이다 (가산점 포인트)
...
- **§2-1의 예외 표가 그대로 적용된다.** LAZY 연관을 트랜잭션 밖에서 건드리면 `LazyInitializationException`이다.
- **§2-3의 `equals` 함정이 그대로 적용된다.** 오히려 이쪽이 더 자주 터진다 — 개발자가 `getReferenceById`를 직접 부른 적이 없어도 프록시는 이미 도처에 있기 때문이다.
...
**OSIV 설정에 따라 동작이 갈린다.** Spring Boot 기본값(`spring.jpa.open-in-view=true`)에서는 트랜잭션이 끝난 뒤에도 `EntityManager`가 응답 완료까지 열려 있으므로 **컨트롤러에서 초기화가 되고 코드가 잘 돌아간다.** 문제는 그게 "잘 돌아가는 게 아니라 문제가 숨은 것"이라는 점이다 — 커넥션을 뷰 렌더링까지 붙잡고, 나중에 OSIV를 끄면 그동안 쌓인 모든 지뢰가 `LazyInitializationException`으로 한꺼번에 터진다. 그것도 §2-2의 경우 B처럼 **애플리케이션 코드가 스택에 한 줄도 없는 직렬화 단계**에서 터지므로 진단이 특히 어렵다.
...
그리고 결정적으로, **이 둘은 양자택일이 아니다.** §3-2의 패턴이 바로 그 답이다 — **일괄 검증으로 조기에 읽기 쉽게 드러내고, FK 제약은 그대로 두어 최후 방어선으로 남긴다.** 성능(SELECT 제거)과 진단 가능성(조기 검증)을 둘 다 가져가는 방법이 있는데 한쪽을 포기할 이유가 없다.
...
**IDE가 생성한 `getClass() != o.getClass()` 비교가 프록시에서 깨진다.** 프록시의 실제 클래스는 `Member$HibernateProxy`(Hibernate 5.x라면 뒤에 무작위 접미사가 더 붙는다)라 `Member.class`와 같지 않아서, **같은 행을 가리키는 실제 엔티티와 프록시가 서로 다르다고 판정된다.** `Set`에 중복으로 들어가고 `contains`가 실패한다.

처방은 **`instanceof` 기반 비교** 또는 **실질 클래스 비교**(`HibernateProxy`면 `getHibernateLazyInitializer().getPersistentClass()`)다. 두 가지를 함께 지켜야 한다 — **상대 객체의 필드를 직접 읽지 말고 `getId()`로 읽을 것**(프록시의 필드는 비어 있다), **`hashCode`는 `id`에 의존하지 않는 고정값일 것**(저장되며 `id`가 채워지면 해시가 바뀌어 컬렉션에서 못 찾게 된다). `Hibernate.unproxy()`나 `Hibernate.getClass()`도 정답이지만 **프록시를 초기화하므로 `equals` 안에서 쓰면 의도치 않은 SELECT를 부른다.** 더 자세한 내용은 [엔티티의 equals/hashCode](25-entity-equals-and-hashcode.md)에 있다.
