...
여기서 **`getReferenceById`와 OSIV 설정이 서로 얽혀 있다**는 점이 중요하다. `open-in-view=true`(Spring Boot 기본값)는 트랜잭션이 끝난 뒤에도 `EntityManager`를 응답이 끝날 때까지 열어 두므로, **위 before 코드가 "잘 돌아간다".** 그러다 커넥션 고갈을 잡으려고 OSIV를 끄는 순간 **여태 잘 돌던 화면들이 `LazyInitializationException`으로 무너진다.** 상세한 전환 절차는 [OSIV 트레이드오프와 전환](osiv-tradeoff-and-migration.md)에, 예외 자체의 처방 범위는 [LazyInitializationException](lazy-initialization-exception.md)에 정리되어 있다.
...
정리하면 **`getReferenceById`는 "LAZY 연관에서 자동으로 일어나는 일을 개발자가 직접 손으로 하는 것"**이다. 자세한 내용은 [EAGER vs LAZY 페치 전략](eager-vs-lazy-fetch-strategy.md)과 [N+1 탐지와 해결](n-plus-one-detection-and-fixes.md)에 있다.
...
그래서 처방은 예외를 잡는 것이 아니라 **경계를 세우는 것**이다. **프록시는 트랜잭션 안에서 소비하고, 밖으로 나가는 것은 DTO뿐**으로 규칙을 정하고, `open-in-view=false` 프로파일로 도는 통합 테스트를 하나 붙여 **기계가 이 규칙을 지키게** 한다. (자세히는 [OSIV 트레이드오프와 전환](osiv-tradeoff-and-migration.md).)
