...
**옆 문항과의 경계**: [컬렉션 fetch join + 페이징](../03-jpa-orm/fetch-join-pagination-in-memory.md)은
...
> **(created_at, PK)만** 있다([세컨더리 인덱스 구조](clustered-vs-secondary-index.md))
...
I/O보다 비싼가](index-not-used-full-scan.md)의 비용 모델이 그대로 곱해진다.
...
인덱스 리프만 순차로 읽는다**([커버링 인덱스](covering-index.md)). 100,020개 엔트리를 세며
...
[Querydsl 동적 쿼리](../03-jpa-orm/querydsl-dynamic-query-composition.md)에서
