...
상태"를 기록하는 영역이다([11-mvcc-innodb.md](./11-mvcc-innodb.md) 2-2). 행 하나를
...
> ([23-read-replica-routing-and-lag.md](../03-jpa-orm/23-read-replica-routing-and-lag.md) 7장).
...
for f in $(git diff --name-only origin/main -- 'src/main/resources/db/migration/*.sql'); do
  if grep -qiE 'CREATE TABLE [`"]?[a-z_]+_(log|history|event)s?[`"]?' "$f" \
    ...
    echo "::error file=$f::이벤트성 테이블은 COMMENT에 retention=<기간>;purge=<방식>을 명시해야 합니다 (knowledge/04-rdb-sql/25-mass-delete-archiving-and-partitioning.md 4-5)"
    exit 1
  fi
done
```
