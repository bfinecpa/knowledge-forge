...
데드락은 [갭 락과 데드락](12-gap-lock-next-key-lock-deadlock.md)에 맡기고 링크만 건다.
...
[클러스터드 인덱스의 리프](03-clustered-vs-secondary-index.md)에만 있고,
...
[갭 락과 데드락](12-gap-lock-next-key-lock-deadlock.md)에서 다룬다. 격리 수준 자체는
...
[03 문서 §4](../03-jpa-orm/10-optimistic-vs-pessimistic-lock.md)가 다룬
"얼마나 오래 잠그느냐"가 이 축이고, 트랜잭션이 길어질 때 락 점유 외에
무엇이 더 무너지는지는 [긴 트랜잭션의 해악](16-long-transaction-harm-and-shortening.md)에서
...
[풀스캔 원인 6종](02-index-not-used-full-scan.md)이 그대로 `FOR UPDATE`에 붙는다.
...
팀 규칙으로 두면([EXPLAIN 읽는 법](10-explain-and-slow-query-process.md)), 사고의 절대다수(인덱스 없음·형변환·선택도)가 리뷰 단계에서
...
일반 SELECT는 잠금 읽기가 아니라 **MVCC 스냅샷 읽기**([MVCC](11-mvcc-innodb.md))라
