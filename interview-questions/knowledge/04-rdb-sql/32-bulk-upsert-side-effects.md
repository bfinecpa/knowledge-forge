...
판정할 수 있다([유니크 제약 문서 §5-2](../03-jpa-orm/14-unique-constraint-concurrent-insert.md)).
...
없다(§8). 상세는 [유니크 제약 문서 §5-4](../03-jpa-orm/14-unique-constraint-concurrent-insert.md).
...
[`03-clustered-vs-secondary-index.md`](03-clustered-vs-secondary-index.md). 그래서 순서는
...
이 한 줄이 [갭 락 문서 §3-2 패턴 B](12-gap-lock-next-key-lock-deadlock.md)와의 차이를
...
만든다 — 3장에서 본 **락 순서 역전**([갭 락 문서 §3-3](12-gap-lock-next-key-lock-deadlock.md))이
...
5. **재시도 폭풍** — 데드락 희생자가 지터 없이 곧바로 재시도해 같은 박자로 다시
   충돌한다([갭 락 문서 §5-4](12-gap-lock-next-key-lock-deadlock.md)).
...
교차 대비). 대가는 스큐와 커넥션 수([커넥션 수 문서](15-connection-count-vs-throughput.md)).
...
외부 채번(Snowflake)은 [`26-unique-id-generation-at-scale.md`](26-unique-id-generation-at-scale.md).
...
줄어든다([대량 INSERT 문서 §5](../03-jpa-orm/21-bulk-insert-jdbc-batch.md) — 이 설정은
...
세 규칙은 [갭 락 문서 §5-4](12-gap-lock-next-key-lock-deadlock.md)와 같다 — **트랜잭션
...
7. **JPA를 우회한다** — 네이티브 문장이라 1차 캐시·Auditing·`@Version`·리스너가
   동작하지 않는다([유니크 제약 문서 §5-3](../03-jpa-orm/14-unique-constraint-concurrent-insert.md)).
