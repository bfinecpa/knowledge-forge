...
> [`../03-jpa-orm/read-replica-routing-and-lag.md`](../03-jpa-orm/read-replica-routing-and-lag.md)에서
...
- **CPU**: MySQL에서 쿼리 하나는 스레드 하나가 실행한다. 수백만 행을
  ...
  ([`connection-count-vs-throughput.md`](connection-count-vs-throughput.md) §2 사슬 A).
...
[`mvcc-innodb.md`](mvcc-innodb.md) §3에서 이미 고정했다 — 여기서는 "그
...
- **커버링 인덱스**: `(seller_id, paid_at, amount)` 같은 인덱스가 있으면
  ...
  사슬 A·B가 크게 준다 ([`covering-index.md`](covering-index.md)).
- **불필요한 조인 제거**: 위젯이 `order_items`를 안 보여주는데 조인돼
  있는 경우가 흔하다. EXPLAIN으로 rows를 확인한다
  ([`explain-and-slow-query-process.md`](explain-and-slow-query-process.md)).
...
[`normalization-vs-denormalization.md`](normalization-vs-denormalization.md)
...
[`../02-spring/scheduled-tasks-threading.md`](../02-spring/scheduled-tasks-threading.md)
