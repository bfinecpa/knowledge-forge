...
> 구멍](../03-jpa-orm/jpa-auditing-and-bulk-gap.md)에 있다 — 그 문서의
...
DDL 문제다([무중단 DDL](./online-ddl-zero-downtime-schema-change.md)).
...
- `ON UPDATE CURRENT_TIMESTAMP`는 **DB 레벨**이라 벌크 UPDATE, 네이티브
  ...
  구멍](../03-jpa-orm/jpa-auditing-and-bulk-gap.md)). 단 **다른 컬럼 값이
...
2. **표시는 사용자 타임존, 변환은 가장 바깥에서.** 사용자 존은
   ...
   탄다**([인덱스가 있는데 풀스캔](./index-not-used-full-scan.md)) — "오늘
...
DDL](./online-ddl-zero-downtime-schema-change.md)의 백필 절차와 같다).
