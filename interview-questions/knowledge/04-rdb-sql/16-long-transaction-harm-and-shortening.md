...
MVCC의 부품(숨은 컬럼·undo 체인·Read View)은 [11-mvcc-innodb.md](./11-mvcc-innodb.md),
undo/redo의 기본 역할은 [05-transaction-acid.md](./05-transaction-acid.md)를
...
[11-mvcc-innodb.md](./11-mvcc-innodb.md) §2~§3에 있다. 이 문서에서는 (a)를
...
- **메타데이터 락(MDL)** — 이건 `SELECT`도 잡는다. 트랜잭션이 테이블을 한
  ...
  **그 뒤에 오는 모든 쿼리(단순 SELECT조차)가 ALTER 뒤에 줄을 선다.**
  ...
  [14-online-ddl-zero-downtime-schema-change.md](./14-online-ddl-zero-downtime-schema-change.md) §1-1·§4-1.
...
곧 라우팅 조건이 된다 — [13-transactional-readonly-optimization.md](../02-spring/13-transactional-readonly-optimization.md).
