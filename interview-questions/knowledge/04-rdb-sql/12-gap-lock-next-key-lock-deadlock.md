...
문서](../03-jpa-orm/15-flush-timing-and-sql-ordering.md), [자바 데드락 문서
§4](../01-java-kotlin/24-deadlock-analysis-prevention.md)). 갭 락 데드락은
...
§3](../03-jpa-orm/14-unique-constraint-concurrent-insert.md)에 정리돼 있다.
...
§8](../03-jpa-orm/14-unique-constraint-concurrent-insert.md)의 "동시 출발"
...
유니크 제약 문서 §3](../03-jpa-orm/14-unique-constraint-concurrent-insert.md)).
...
- **지터(random backoff)** — 두 요청이 같은 간격으로 재시도하면 같은 박자로
  다시 충돌한다. 자바 데드락 문서의 `randomBackoff()`와 같은 이유([§5](../01-java-kotlin/24-deadlock-analysis-prevention.md)).
