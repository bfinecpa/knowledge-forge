...
## 2. Dead Letter Exchange와 그 위에 얹는 지연 재시도

이 절은 장치 하나와 그 장치로 조립하는 패턴 하나를 이어서 본다. 먼저 RabbitMQ가 브로커 기능으로 제공하는 **Dead Letter Exchange(DLX)** — 실패한 메시지가 자동으로 흘러가는 옆길 — 이 무엇이고 어떤 조건에서 발동하는지 보고(2-1~2-3), 그다음 그 옆길을 원래 큐 쪽으로 되돌려 놓아 만드는 **지연 재시도**를 본다(2-4).

순서를 이렇게 잡은 이유는 지연 재시도가 별개의 기능이 아니라 **DLX를 재료로 조립한 결과물**이기 때문이다. DLX의 발동 조건 세 가지를 모른 채 지연 재시도 구성도부터 보면 화살표 몇 개가 왜 그렇게 이어지는지가 안 보인다. 기본 장치를 먼저 깔고 그 위에 패턴을 얹는다.
...
### 2-4. 지연 재시도 — TTL과 DLX를 엮는 표준 패턴

여기서부터가 2-1~2-3의 장치를 재료로 쓰는 부분이다. 특히 2-1 표의 두 번째 행 — **TTL이 만료되면 메시지가 DLX로 넘어간다** — 하나만 뒤집어 쓰면 브로커가 타이머 역할을 하게 만들 수 있다.
...
spring:
  rabbitmq:
    listener:
      simple:
        default-requeue-rejected: false   # 무한 루프 방지 (3절)
        retry:
          enabled: true
          max-attempts: 3
...
## 3. 재시도를 설계하지 않으면 — requeue 무한 루프와 그 뒤처리

2절이 "실패 경로를 제대로 설계했을 때"의 그림이라면 이 절은 그 반대편이다. 재시도 경로를 설계하지 않은 컨슈머에게 실제로 벌어지는 사고와, 그 사고가 지나간 뒤 남은 메시지를 어디에 두고 어떻게 되살리는가를 **시간순으로** 이어서 본다.

앞쪽(3-1~3-3)은 사고 자체다. `requeue=true`의 무제한 반복이 어떻게 CPU를 태우는지, 어떻게 막는지, 어떻게 알아채는지. 뒤쪽(3-4)은 사고 이후다. 상한에 걸려 빠져나온 메시지가 모이는 DLQ를 어떻게 운영하는가. 둘을 한 절에 둔 이유는 뿌리가 같기 때문이다 — **"실패한 메시지를 어디로 보낼지 정해두지 않았다"**는 하나의 누락이 앞에서는 무한 루프로, 뒤에서는 되살릴 수 없는 유실로 나타난다.

### 3-1. requeue 무한 루프는 어떻게 만들어지는가
...
**첫째, 기본값을 바꾼다.** `default-requeue-rejected: false`로 두면 실패한 메시지가 DLX로 넘어간다. 재시도가 필요하면 2-4의 대기 큐 구조를 명시적으로 설계한다.
...
### 3-4. DLQ 운영 — 버리는 곳이 아니라 병원이다

무한 루프를 끊고 상한을 걸어두면 실패한 메시지는 결국 DLQ에 모인다. 여기서부터가 사고 이후의 국면이다.

이때 DLQ를 "실패한 메시지를 버리는 쓰레기통"으로 취급하면 앞에서 애써 격리한 의미가 없어진다. **DLQ는 병원에 가깝다** — 환자를 눕혀 두는 곳이고, 무엇 때문에 쓰러졌는지 적힌 진단서가 함께 있어야 하며, 나을 수 있는 환자를 내보내는 퇴원 절차가 미리 마련되어 있어야 한다. 아래 셋이 그 진단서·감시·퇴원 절차에 각각 대응한다.
