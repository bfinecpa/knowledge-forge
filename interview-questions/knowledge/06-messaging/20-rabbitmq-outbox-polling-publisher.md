...
세 번째는 `@Transactional` 안에서 발행이 예외를 던지면 롤백되므로 막히는 것처럼 보인다. 그런데 **발행이 예외를 던지지 않으면서 실패할 수 있다**는 것이 함정이다. `convertAndSend`는 소켓에 써 넣고 곧바로 리턴하므로 브로커가 그 메시지를 실제로 받았는지는 확인하지 않는다. 브로커가 내려가 있거나 도중에 연결이 끊기면 메시지는 조용히 사라지는데 커밋은 정상적으로 끝난다 — 세 번째다. (이 "리턴했다고 도착한 것이 아니다"라는 성질은 3-3에서 publisher confirm으로 다시 다룬다.)

네 번째는 발행 호출과 커밋의 **순서** 때문에 생긴다. `@Transactional` 메서드에서 커밋은 메서드가 끝난 뒤에 일어나므로, ②까지 마쳐놓고 커밋 직전에 프로세스가 죽거나 커밋 자체가 실패하면(DB 커넥션 끊김, 제약 위반) **발행은 나갔는데 DB는 롤백된** 상태가 된다.
...
이제 실제로 만들어 본다. 이 절은 두 부분으로 이어진다.

앞쪽(3-1~3-4)은 **동작하게 만드는 부분**이다. 테이블을 어떻게 잡고, 여러 인스턴스가 같은 행을 집지 않게 하고, publisher confirm과 어떻게 엮고, 폴링 주기 때문에 생기는 지연을 어떻게 줄이는가.

뒤쪽(3-5)은 **그 구현을 실제 서비스에 올렸을 때 반드시 따라오는 부분**이다. outbox 테이블은 발행이 끝난 행까지 계속 쌓여 자라고, 퍼블리셔는 아무 예외도 남기지 않고 조용히 멈출 수 있고, 몇 번을 재시도해도 발행되지 않는 행이 반드시 생긴다. 이 둘을 떼어놓고 앞쪽만 만들면 **"돌아가기는 하는데 한 달 뒤에 터지는"** 구현이 되므로 한 흐름으로 이어서 본다.
...
먼저 용어부터 짚는다. **publisher confirm은 "이 메시지를 브로커가 확실히 받아 책임진다"고 발행자에게 돌려주는 브로커의 응답**이다. AMQP의 기본 발행은 응답이 없는 단방향 전송이라 발행 메서드가 리턴해도 그것은 "소켓에 썼다"는 뜻밖에 되지 않는다. confirm 모드를 켜면 브로커가 메시지를 받아 (persistent 메시지라면 디스크에 내린 뒤) ack를 돌려준다. 자세한 동작은 `13-rabbitmq-durability-publisher-confirms.md`에서 다룬다.
...
RabbitTemplate outboxRabbitTemplate(ConnectionFactory cf, OutboxRepository repo) {
    ...
    t.setMandatory(true);   // 라우팅 실패도 잡아야 한다 (아래 3-5)
...
**그리고 아무리 정교하게 해도 중복을 완전히 없앨 수는 없다.** 브로커가 받고 confirm을 보내는 도중 네트워크가 끊기면 "도달 못 함"과 "응답만 유실"을 구분할 방법이 없다. **이 구조는 본질적으로 at-least-once이며, 멱등 컨슈머가 필수 전제**다(소비 측에서 중복을 걸러내는 구현은 `04-idempotent-consumer-implementation.md`에서 다룬다).
...
3-1~3-4까지로 발행은 동작한다. 여기서부터는 그 구현이 몇 달 동안 살아 있게 만드는 부분이다. 네 가지가 있고, 전부 빠뜨렸을 때 실제로 사고가 나는 지점이다.
