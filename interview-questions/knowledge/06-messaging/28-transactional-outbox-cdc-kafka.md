...
여기서 `msg.eventId()`가 무엇인지가 깔끔하다. **outbox 테이블의 `id`(bigserial PK)가 그대로 이벤트 고유 ID**가 된다. 발행 경로와 멱등 키 체계가 한 번에 정리되는 조합이다(`04-idempotent-consumer-implementation.md`).
...
> **RabbitMQ에서의 대응**: Debezium CDC 같은 정석 경로가 없어 `FOR UPDATE SKIP LOCKED` 폴링 퍼블리셔 + publisher confirm 콜백이 실무 기본형이다. `20-rabbitmq-outbox-polling-publisher.md`에서 다룬다.
